...
xmask = np.logical_and(x > 0.4, x < 1.4)
ymask = np.logical_and(y > 0.5, y < 0.58)
mask = np.logical_and(xmask,ymask)

#%%
selection = u0.select_atoms('all')
with mda.Writer('unclamp.xtc', selection.n_atoms) as W:
...
tik = time()
u = mda.Universe(f'{traj_dir}/jR2R3_P301L.gro','unclamp.xtc')
pdb = mda.Universe('unclamp_cluster1.pdb')
...
pc_values = create_pc_values(-10,10)
print(f'# of frames: {len(pc_values)}')

protein_positions = cluster_backbone.positions
# make a directory 'unclamp_pdb_snapshots' to store snapshots
if not os.path.exists('unclamp_pdb_snapshots'):
    os.makedirs('unclamp_pdb_snapshots')
...
for i,pc_value in enumerate(pc_values):
    ...
    cluster_backbone.write(f"unclamp_pdb_snapshots/pc{which_pc}_snapshot{i}.pdb".format(pc_value))

pdb_files = [f"unclamp_pdb_snapshots/pc{which_pc}_snapshot{snapshot}.pdb".format(pc_value) for snapshot in np.arange(i+1)]

trajs = [md.load(pdb) for pdb in pdb_files]
stitched_traj = md.join(trajs)
stitched_traj.save(f'unclamp_pc{which_pc}.pdb')
...
u_new = mda.Universe(f'unclamp_pc{which_pc}.pdb')
# select the first pair of atoms (x-axis)
pair1a = u_new.select_atoms('resid 298 and name CA'); pair1b = u_new.select_atoms('resid 307 and name CA')
pair2a = u_new.select_atoms('resid 300 and name CA'); pair2b = u_new.select_atoms('resid 305 and name CA')
dist1s = []; dist2s = []
...
plt.show()
ani.save("unclamp_just_landscape.gif",dpi=200)
