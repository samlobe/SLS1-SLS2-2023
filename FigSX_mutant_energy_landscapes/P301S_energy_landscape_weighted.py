...
equilibration_times = [50,50] #ns for HIE HID

weights = [0.5,0.5] # weights for each uncharged histidine tautomer

dist_dir = '../pair_distances'
molecules = ['jR2R3_P301S_HIE','jR2R3_P301S_HID'] # HIE, HID
title = 'jR2R3 P301S'
#title = 'jR2R3 P301S \n(50% H299-HID; 50% H299-HIE)'

equilibration_times = [50,50]
...
df1 = load(f'{dist_dir}/pair_distances_{molecules[0]}.csv',equilibration_times[0],10,10) # converged by 50ns
df2 = load(f'{dist_dir}/pair_distances_{molecules[1]}.csv',equilibration_times[1],10,10)

distx = 'Distance: LYS298 and GLN307'
disty = 'Distance: VAL300 and SER305'

x1 = df1[distx].to_numpy()
y1 = df1[disty].to_numpy()
x2 = df2[distx].to_numpy()
y2 = df2[disty].to_numpy()

# combine the data and use the right weights
x = np.concatenate((x1,x2))
y = np.concatenate((y1,y2))
weights = np.concatenate((np.ones(len(x1))*weights[0]/len(x1),np.ones(len(x2))*weights[1]/len(x2)))
...
plt.tight_layout()
fig.savefig('P301S_energy_landscape_2D.png',dpi=300,bbox_inches='tight')
